        ...
        {invites.length === 0 ? (
          ...
          <div className="space-y-px overflow-hidden rounded-ir-card bg-ir-border">
            {invites.map((invite) => {
              ...
              return (
                <div
                  // `card` marker — safe here because this row already
                  // carries its own explicit `flex flex-col`; `rounded-none`
                  // cancels card's own border-radius since these rows are
