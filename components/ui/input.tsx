...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        // `input` (DaisyUI) is layered in as the base marker class, same
        // technique as Button — its own width/height/padding/radius/font-size
        // are all overridden by our later-generated w-full/h-10/px-3/py-1/
        // rounded-ir-input/text-base utilities below (see button-variants.ts
        // for the full explanation), so pixel output is unchanged.
